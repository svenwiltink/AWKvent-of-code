...
# encodes an instructions and state array to a string
...
# decodes a string back into the provided instructions and state variables
...
# variables contains the variables to read when opcode 3 is eninstructionCountered
# pState will contain the program instructionCounter (pc), relative instructionCounter (rc) and HALT keys when returned
# pState can be fed back into startIntCode to resume a previous state
function startIntCode(pData, options, variables, pState) {
	pc = int(pState["pc"])
	rc = int(pState["rc"])
	variableIndex = 0
	outputString = ""
	...
	while (pc < instructionCount) {
		...
		if (opcode == 1) {
			...
			if (intDebug) printf "storing %d in %d\n", newValue, location
		...
		if (opcode == 4) {
			if (intDebug) print ""
			if (intDebug) printf "DIAGNOSTICS %d\n", p1
			if (intDebug) print ""
		...
		if (opcode == 6) {
			if (p1 == 0) {
				if (intDebug) printf "jumping to %d because %d == 0\n", p2, p1
				pc = p2
				continue
			}
		...
		if (opcode == 9) {
			rc += p1
			if (intDebug) printf "increasing rc with %d, now %d\n", p1, rc
			pc += 2
		}			
	}
}