...
function startWithSettings(settings) {
	...
	for (settingsString in generatedSettings) {
		...
		for (i=1; i<= 5; i++) {
			...
			if (i == 1) ampOutput = startProgram(ampAData, ampVariables)
			if (i == 2) ampOutput = startProgram(ampBData, ampVariables)
			if (i == 3) ampOutput = startProgram(ampCData, ampVariables)
			if (i == 4) ampOutput = startProgram(ampDData, ampVariables)
			if (i == 5) ampOutput = startProgram(ampEData, ampVariables)
		}
...
function startProgram(ampData, variables) {
	pc = 0
	variableIndex = 0
	outputString = ""
